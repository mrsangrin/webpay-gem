...
# Utility class
class Util
  def self.xml_sign!(xml_request, certs = {})
    document = Nokogiri::XML(xml_request.body)
    ...
    signer.cert = OpenSSL::X509::Certificate.new(certs[:public_cert])
    signer.private_key = OpenSSL::PKey::RSA.new(certs[:private_key])

    signer.document
          .xpath('//soapenv:Body', 'soapenv' => 'http://schemas.xmlsoap.org/soap/envelope/')
          .each do |node|
      signer.digest!(node)
    end
